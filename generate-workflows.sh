...
main() {
    ...
    for i in "${!workflows[@]}"; do
        ...
        {
            sed -e "s/%%NAME%%/$kernel_flavor ($version)/;s/%%KERNEL_FLAVOR%%/$kernel_flavor/;s/%%VERSION%%/$version/" ./workflow-templates/workflow.yaml.in
            sed -e "s/%%NAME%%/$kernel_flavor ($version)/;s/%%KERNEL_FLAVOR%%/$kernel_flavor/;s/%%VERSION%%/$version/" ./workflow-templates/cache_kernel.yaml.in
            for j in common extra nvidia nvidia-open zfs; do
                ...
                sed "s/%%VERSION%%/$version/;s/%%KERNEL_FLAVOR%%/$kernel_flavor/;s/%%AKMODS_TARGET%%/$akmods_target/" "./workflow-templates/job.yaml.in"
            done
        } > "./.github/workflows/build-$i.yml"
    done
}
